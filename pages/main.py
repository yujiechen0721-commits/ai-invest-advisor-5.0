...
if not btn_start and 'analyzed' not in st.session_state:
    ...
    with t1:
        ...
        with c1:
            ...
            fig_pie = go.Figure(data=[go.Pie(
                ...
                hole=.45,
                textinfo='percent',  # <--- 修改點：只顯示百分比數字
                insidetextfont=dict(size=24, color='white', family="Arial Black"), # 加大並加粗數字
    ...
    with t3:
        ...
        rc3.markdown(f'<div class="glass-card" style="text-align:center;"><h5>風險評級</h5><h2 style="color:#fbbf24;">{"低" if u_risk < 4 else "中" if u_risk < 8 else "高"}</h2></div>', unsafe_allow_html=True)
    ...
    with t4:
        ...
        with col_a:
            with st.expander("📊 0050.TW 元大台灣50"): st.write("代表台灣競爭力最強的 50 家龍頭企業。")
            with st.expander("🌍 VT 全球股票 ETF"): st.write("涵蓋全球超過 9,000 檔股票。")
        with col_b:
            with st.expander("🛡️ BND 全球債券 ETF"): st.write("超過 60% 為 AAA 級政府債。")
            with st.expander("💰 0056.TW 元大高股息"): st.write("適合現金流需求者。")
